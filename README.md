...
+ в модуле **read_files** объявлены функции для получения данных по транзакциям пользователя из файла формата CSV или Excel.
+ в модуле **extra_service** реализована функция поиска операций по описанию и подсчета операций по заданным категориям.
...
+ Проверено, что функция получает доступ к файлу по указанному пути и преобразует данные в список транзакций.
+ Тестирование случаев, когда дан некорректный путь к файлу, либо неверный формат для чтения. Функция возвращает пустой список с указанием ошибки.

Функция `read_excel_file`:
+ Проверено, что функция получает доступ к файлу по указанному пути и преобразует данные в список транзакций.
+ Тестирование случаев, когда дан некорректный путь к файлу, либо неверный формат для чтения. Функция возвращает пустой список с указанием ошибки.

**Модуль extra_service**

Функция `search_by_string`:
+ Проверено, что функция возвращает список операций по заданной строке поиска, проверяя в поле описания.
+ Тестирование случаев некорректного описания для поиска или не найденного в списке операций.

Функция `count_operations_by_categories`:
+ Проверено, что функция возвращает словарь с подсчетом операций по заданному пользователем списку категорий из поля описания.
+ Тестирование случаев, когда заданная категория пустая или не в списке операций.
